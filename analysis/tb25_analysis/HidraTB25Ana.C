/*********************************************/
/*  HidraTB25Ana.C                           */
/*Analysis file for TB25 HidraSim simulation */
/* Contact: A. Pareti (andrea.pareti@cern.ch)*/
/*********************************************/
#include <TTree.h>
#include <TFile.h>
...
const double sci_pde = 0.22; // increased wrt to datasheet due to higher overvoltage in TB25
const double cer_pde = 0.38; // increased wrt to datasheet due to higher overvoltage in TB25

// Containment correction factors
...
// Include/exclude: - saturation, noise, pedestal subtraction
// Noise parameters: - noise correlation mode, noise distribution
// Can also scale pedestal subtraction to try matching observed data linearity
...
const double ScalePedestalSubtractionFactorS = 1.0;  
...
// FERS activation values (pedestal subtracted) -> to be used
...
// -> preliminary, measured threhsold values after pedestal subtraction with muons
std::map<unsigned int, double> fers_to_thr_map = {
{1, 0.2225},
{2, 0.24},
{3, 0.3508},
...
void GetSiPMcoordinate(int TowID,
                       ...
                       unsigned int groupingValue)
{
  const double towerPitchY = dtubeY * NofFibersrow;
  double TowerOffsetY = -((NoModulesSiPM - 1) * towerPitchY) / 2 + TowID * towerPitchY;
...
void HidraTB25Ana(double energy, const std::string& input)
{
  ...
  const std::string sipmMapPath =
    "../../../TBDataPreparation/2025_SPS/MapAndCalibration/sipm_map.json";

  const std::string pedestalJsonPath =
    "../../../TBDataPreparation/2025_SPS/MapAndCalibration/SiPM_pedestals_v1.json";

  const std::string adcToGeVJsonPath =
    "../../../TBDataPreparation/2025_SPS/MapAndCalibration/SiPM_ADCtoGeV_v1.json";
  ...
  auto h2_SvsC = new TH2F("SvsC", "S vs C energy (all fibres); S/E; C/E", 100, 0., 1.5, 100, 0., 1.5);
